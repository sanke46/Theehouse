class TableauPile(var cards: MutableList<Card> = mutableListOf()) {
    ...
    fun addCards(newCards : MutableList<Card>):Boolean {
        if (cards.size > 0) {
            if (newCards.first().value == cards.last().value - 1 && suitCaheck(newCards.first(), cards.last())) {
                suitCaheck(newCards.first(), cards.last())
    ...
    private fun suitCaheck(c1: Card, c2: Card): Boolean {
